...
function pickGame24Numbers() {
  for (let attempt = 0; attempt < 80; attempt += 1) {
    const nums = [];
    for (let i = 0; i < 4; i += 1) {
      nums.push(randomInt(10) + 1);
    }
    if (game24HasSolution(nums)) return nums;
  }
  return [1, 2, 3, 4];
}
  ...
  for (let i = 0; i < nums.length; i += 1) {
    for (let j = i + 1; j < nums.length; j += 1) {
      ...
      for (const v of candidates) {
        if (game24HasSolution([...rest, v])) return true;
      }
    }
  }
  return false;
...
function dealGame24() {
  game24Numbers = pickGame24Numbers();
  renderGame24Numbers();
  clearGame24Input();
  hideGame24Msg();
}
...
function evaluateGame24Tokens(tokens) {
  ...
  function parseTerm() {
    ...
    while (pos < tokens.length && (tokens[pos].value === "*" || tokens[pos].value === "/")) {
      ...
      if (op === "/" && Math.abs(right) < 1e-9) return NaN;
      left = op === "*" ? left * right : left / right;
    }
    return left;
  }
